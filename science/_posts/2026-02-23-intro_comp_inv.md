...
The likelihood (the 'probably' part) can be thought of as sampling over possible training sets, while the closeness (the 'approximately' part) can be thought of as an error metric on test sets, usually assumed to have the same distribution as the training set.
To determine the hardness of a learning problem, we ask how the number of samples that we need scales with the likelihood, the closeness, and the size of the observed system.
...
This can be unintuitive for people who are used to thinking about different models and learning approaches, but it again parallels more traditional computational complexity results about the difficulty of problems overall, as distinct from individual algorithms to solve those problems.
...
If the particular system that we are studying actually falls into a category that is difficult to learn about, then it will _require_ a (potentially impractically) large amount of data to learn the features of that system, regardless of how it is fit.
...
(Stated more formally, there won't be enough data to be _reasonably sure_ that the fit models capture behaviors of interest _reasonably well_.)
...
Starting from an initial internal state, the machine observes a series of inputs, which cause the internal state to transition to a new state, depending on the input and the current state of the machine.
It then produces an output (usually either accepting or rejecting the input) based on the final state.
The _learning problem_ is to learn a finite automaton (i.e. initial state, the state/input-dependent transition matrix, and the state-dependent outputs) that mimics the behavior of the observed automaton.
...
Classical examples of this approach include action in physics and utility maximization in economics.
